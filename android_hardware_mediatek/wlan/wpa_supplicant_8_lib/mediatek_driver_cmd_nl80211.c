...
static int testmode_reply_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    strncpy(reply_info->reply_buf.ptr, nla_data(tb[NL80211_ATTR_TESTDATA]), reply_info->reply_buf_size - 1);
...
static int wpa_driver_nl80211_testmode(void *priv, const u8 *data, size_t data_len)
{
    ...
    switch(index) {
        ...
        case NL80211_TESTMODE_STR_CMD:
        {
            ...
            return send_and_recv_msgs(drv, msg, testmode_reply_info_handler, &(reply_params->reply_info), NULL, NULL);
        }
        default:
        {
                  ...
                  size_t buf_len)
{
    ...
    if (os_strncasecmp(cmd, "POWERMODE ", 10) == 0) {
        ...
        params.hdr.index = params.hdr.index | (0x01 << 24);
        params.hdr.buflen = sizeof(params);
        os_memcpy(params.bssid, entry->bssid, IW_ETH_ALEN);
        os_memcpy(params.sta, entry->sta, IW_ETH_ALEN);
        os_memcpy(params.pmkid, entry->pmkid, IW_PMKID_LEN);
        ...
        params->hdr.buflen = sizeof(*params) + strlen(cmd) + 1;
        params->reply_info.reply_buf_size = buf_len;
        params->reply_info.reply_len = 0;
        params->reply_info.reply_buf.ptr = buf;
        memcpy((char*)(params + 1), cmd, strlen(cmd) + 1);
        result = wpa_driver_nl80211_testmode(priv, buffer, params->hdr.buflen);
        ret = params->reply_info.reply_len;
        handled = result != -EOPNOTSUPP;
        if (handled && result != 0 && ret == 0) {
            strncpy(buf, "FAILED", buf_len - 1);
            ret = strlen("FAILED");
        }
        wpa_printf(MSG_INFO, "Transparent command for driver nl80211, result %d, reply_len=%d, reply=%s", result, ret, buf);
    }
...
#ifdef CONFIG_MTK_GENERIC_EVENT
...
#endif /* CONFIG_MTK_GENERIC_EVENT */
